<p align="center">
  <img src="./assets/banner.svg" alt="XXD Panel 023 프로젝트 배너" width="1200">
</p>
...
[![Four Modes](https://img.shields.io/badge/Modes-4-d75d32?style=flat-square)](#네-가지-출력-하나의-동양-창경-논리)
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 023 예시 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 023 예시 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 023 예시 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 023 예시 8"></td>
  </tr>
</table>

## 추가 3:4 상하 구성 예시

위 16:9 예시와 같은 네 개의 독립 소스를 완전한 3:4 상하 캔버스로 다시 생성했습니다. 위쪽은 현실 사진을 유지하고 아래쪽은 Panel 023의 원본 프롬프트에 따라 재구성합니다. 영어 문구는 각 사진의 내용·분위기·함의에서만 생성했으며 모든 결과를 개별 검수했습니다.

<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 023 추가 상하 예시 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 023 추가 상하 예시 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 023 추가 상하 예시 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 023 추가 상하 예시 4"></td>
  </tr>
</table>
...
> 원본이 고른 창 · 옅고 숨 쉬는 배경 · 부드러운 유색광 · 분사 입자 · 흐린 투영과 미세 조판

XXD Panel 023는 Codex와 호환 에이전트를 위한 이미지 생성 Skill입니다. 사진의 윤곽, 동작 방향, 개구부, 무게중심과 공간 관계에 따라 원형창, 화창, 누창, 월동문, 부채창, 육각창 또는 단순 격자 중 하나를 고르고, 주제나 가지 그림자, 빛점, 추상 투영이 창 안에 나타나고 통과하고 가리거나 조금 넘치게 합니다.
...
**Panel 023**은 원본 사진의 핵심을 보존하면서 다음과 같은 시각 방향을 원하는 작업에 적합합니다: **사진이 고른 동양 창과 부드러운 빛으로 고요한 장면 만들기**. 먼저 용도와 원하는 결과를 확인한 뒤 구성, 크기, 텍스트와 전달 형식을 결정합니다.
...
> XXD Panel 023으로 이 이미지를 작업하고, 먼저 가장 어울리는 구성과 크기를 추천해 줘.
...
## 빠른 판단: XXD Panel 023이 나에게 맞을까요?

| 궁금한 점 | 이 스타일이 주는 답 |
|---|---|
| **얻게 되는 결과** | 사진이 고른 동양 창과 부드러운 빛으로 고요한 장면 만들기 |
| **한눈에 알아볼 특징** | 원본이 고른 창 · 옅고 숨 쉬는 배경 · 부드러운 유색광 · 분사 입자 · 흐린 투영과 미세 조판 |
...
```text
윤곽／동작／개구／방향／빛색／관계 고정 → 가장 맞는 전통 창 하나 선택 → 주제나 투영을 나타내기／통과시키기／가리기／넘치게 하기 → 주요 빛 하나＋고요한 여백 하나＋소수의 부드러운 투영 → 원본에서 옅은 배경과 생명의 빛색 추출 → 입자와 파우더와 확산 가장자리로 부드럽게 → 극히 짧은 제목과 미세 문자를 창 가장자리, 곡선, 축, 빛 여백에 배치
```

무관한 사진으로 바꿔도 창 형태, 주제와 창의 관계, 주요 빛, 고요한 여백, 투영, 배경색, 빛색과 문자 경로가 그대로 성립한다면 023가 아닙니다.

## 완성작에서 바로 알아볼 수 있는 스타일 특징

- **원본이 창을 선택:** 최소 세 가지 고유 단서가 창 하나를 결정하며 월동문을 기계적으로 반복하지 않습니다.
- **창과 주제의 상호작용:** 주제, 가지 그림자, 빛점 또는 투영이 창 안에 나타나고 통과하고 가리거나 넘쳐 질서와 깊이를 만듭니다.
- **시각 삼각형 하나:** 주요 빛 하나, 고요한 여백 하나, 소수의 부드러운 투영. 창은 치우치고 잘리거나 떠 있을 수 있습니다.
...
이미지 폴더를 직접 전달하면 이 병사 Skill이 같은 Panel 023의 원본 미학으로 일괄 처리합니다. 일반 래스터 형식을 기본적으로 재귀 탐색하고 수량을 먼저 알린 뒤 모드·크기·텍스트·언어를 한 번만 확인합니다. 각 이미지는 내용이나 문구가 섞이지 않도록 독립 생성하며 전체 배치는 하나의 평면 작업 디렉터리만 사용합니다. 개별 실패는 기록하고 이후 이미지를 조용히 누락하지 않습니다.
...
/xxd-panel-023 photo.jpg --mode top-bottom --size auto --text prompt --locale ko-KR

# 좌우 비교: 3:2 가로형, 글자 없음
/xxd-panel-023 photo.jpg --mode left-right --size 3:2 --text none

# 디자인 전용: 9:16 휴대폰 세로형, 한국어 문구
/xxd-panel-023 photo.jpg --mode design-only --size 9:16 --text prompt --locale ko-KR
...
/xxd-panel-023 photo.jpg --mode design-only --size 3:4 --text prompt --locale ko-KR

# 정확한 문구를 재작성·번역·제목 추가 없이 사용
/xxd-panel-023 photo.jpg --mode design-only --size 3:4 --copy "빛을 이곳에 남겨 두기" --locale ko-KR

# 2개 모드 × 3개 크기로 독립적인 완성 구도 6개 생성
/xxd-panel-023 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ko-KR

# 폴더 전체에 Panel 023 적용, 공통 설정은 한 번만 확정
/xxd-panel-023 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ko-KR
...
Claude Code 사용자는 같은 폴더를 `~/.claude/skills/xxd-panel-023`에 연결할 수 있습니다. 설치 후 에이전트 세션을 다시 시작하세요.
...
- [Skill 워크플로](SKILL.md)
- [중문 런타임 어댑터](references/xxd-panel-023-prompt.zh-CN.md)
- [영문 런타임 어댑터](references/xxd-panel-023-prompt.en.md)
...
**창은 장식이 아니라 주제와 빛과 여백이 함께 일어나는 공간입니다.**
