...
| 모드 | 크기 규칙 | 결과물 |
| --- | --- | --- |
| `top-bottom` | 원본 적응형 | 위에는 전체 원본, 아래에는 023 동양 창과 부드러운 빛의 장면. 두 패널이 원본 크기를 유지하며 정확히 50/50 |
| `left-right` | 원본 적응형 | 왼쪽에는 전체 원본, 오른쪽에는 023 동양 창과 부드러운 빛의 장면. 두 패널이 원본 크기를 유지하며 정확히 50/50 |
| `design-only` | 원본 적응형 | 원본 사진은 보이지 않고 변환 디자인만 표시. 원본 비율과 크기 유지 |
| `wallpaper-pack` | 기기별 네 가지 크기 | 휴대전화, iPad, 데스크톱, 어린이용 스마트워치 PNG 개별 출력 |

정확한 사용자 픽셀 > 지정 비율 또는 용도 > 일반 모드의 원본 적응형 순서입니다. 원본 `023.md`의 3:4는 초기 창작 캔버스일 뿐 현재 Skill의 숨은 기본값이 아닙니다.
...
- **독립형 네 작품:** 각 기기는 원본 사진만 참조하며 창 형태, 주제 관계, 빛 영역, 여백, 흐린 투영과 문자 경로를 각각 탐색합니다.

연결형은 크롭이 아닙니다. 네 장 모두 따로 생성하고 구성하며 검수하고, iPad→휴대전화→데스크톱→스마트워치 순으로 참조를 이어 붙이지 않습니다.

## 문구는 창과 빛의 고요한 질서를 따릅니다

생성 전에 자동 문구, 사용자 지정 문구, 무문자 출력 중 하나를 선택합니다. 문구가 있으면 대상 언어나 지역도 지정합니다.

자동 문구는 사진의 감정, 시간, 동작, 온도 또는 비유에서 극히 짧은 제목을 만들고 필요할 때만 장소어, 상태어 또는 미세 문구를 더합니다. 문자는 창 가장자리, 곡선, 축 또는 빛 여백을 따릅니다.

장소, 날짜, 출처와 사실 번호는 사용자 제공이나 신뢰할 수 있는 근거가 있어야 하며 세련돼 보이려고 꾸며 내지 않습니다. 사용자 확정 문구는 그대로 유지합니다.
...
## 정확한 구성은 코드가, 작품은 이미지 생성이 맡습니다

이미지 모델이 원본에 맞춘 전통 창, 주제와 창의 깊이, 옅은 배경, 생명의 유색광, 분사 입자, 파스텔 가루, 부드러운 초점, 확산 투영과 고요한 미세 문자를 만듭니다. `scripts/compose_panel.py`는 캔버스 계획, 정확한 50/50 래스터 합성, 최종 크기와 검수만 수행하며 프로그램 그림으로 결과물을 위조하지 않습니다.
...
- [Skill 워크플로](SKILL.md)
- [중문 전체 프롬프트](references/xxd-panel-023-prompt.zh-CN.md)
- [영문 전체 프롬프트](references/xxd-panel-023-prompt.en.md)
